(() => {
  const func = (root, initTheme, changeTheme) => {
    ...
    return {
      setTheme: (theme) => {
        if (theme === 'dark' && !isMoved) toggle();
        if (theme === 'light' && isMoved) toggle();
      },
      getTheme: () => (isMoved ? 'dark' : 'light')
    };
  };
